...
class CreateJob(BaseAction):
    ...
    def initialize_params(self):
        ...
        ret_val, self.job_options_dict = self._connector.util.validate_json_object(
            self._action_result, self._param["job_options"], "job_options"
        )
    ...
    def create_job(self):
        ret_val, response = self._connector.util.make_rest_call(consts.METASPONSE_CREATE_JOB, self._action_result, method="post", headers={})
        if phantom.is_fail(ret_val):
            return self._action_result.get_status()
    ...
    def add_options(self):
        for key, value in self.job_options_dict.items():
            ...
            if response.get("validation_errors"):
                ...
                return self._action_result.set_status(phantom.APP_ERROR, f"Error occurred for option id: {key}")
        return phantom.APP_SUCCESS
    ...
    def check_job_status(self):
        ...
        if not response.get("is_ready", False):
            self._action_result.add_data(response)
            return self._action_result.set_status(phantom.APP_ERROR, "Unable to create job, Error: {}".format(response.get("error_msg")))
